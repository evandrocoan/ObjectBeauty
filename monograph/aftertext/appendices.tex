...
A implementação da exibição dos \textit{tokens} no formato do analisador ANTLR 4 foi uma das implementações feitas no fork ``pushdown'' do Analisador Lark (ver \typeref{GrammarsGrammar}).
...
Neste capítulo pode~=se ver na integra o arquivo da gramática (\typeref{grammarsGrammarPushdown}) utilizadas pela adição de cores e
formatação de código~=fonte.
No \typeref{MainFormatterPy,MainHighlighterPy} pode~=se ver o importe explicito deste arquivo de gramática e
seu recebimento pela Analisador Lark (veja \typeref{GrammarsGrammar}).
Nesta gramática pode ser encontrado expressões redundantes como \mintinline{antlr}{enter_block:
OPEN_BRACE} que servem para fazer com que a árvore de sintaxe tenha um nó com o nome ``\mintinline{antlr}{enter_block}'' e
com que o analisador léxico gere um token com o nome ``\mintinline{antlr}{OPEN_BRACE}''.
