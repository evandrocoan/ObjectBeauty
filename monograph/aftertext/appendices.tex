...
A implementação da exibição dos \textit{tokens} no formato do analisador ANTLR 4 foi uma das implementações feitas no \textit{fork} ``pushdown'' do Analisador Lark (Veja a \fullref{section:GrammarsGrammar}).
...
\item ``\mintinline{text}{='comment.line.sma'}'' é o lexema (Veja a \fullref{section:compiladoresEtradutores}) ou
...
quanto para a escrita do \typeref{code:codeFormatterPy},
por consequência,
...
Ambas as figuras \typeref{figure:MainFormatterSyntaxTree,figure:MainFormatterAbstractSyntaxTree} quanto o \typeref{code:MainFormatterPyResult} representam os mesmos dados,
...
\begin{code}
\caption{Arquivo HTML gerado pelo programa de exemplo ``\mintinline{text}|main_formatter.py|''}
...
\begin{figure}[!htb]
\caption{Árvore Sintática ``\mintinline{text}|main_formatter_syntax_tree.png|''}
...
\begin{figure}[!htb]
\caption{Árvore Sintática Abstrata ``\mintinline{text}|main_formatter_abstract_syntax_tree.png|''}
...
\begin{code}
\caption{Resultado da execução do arquivo ``\mintinline{text}|source/main_formatter.py|''}
...
Ambas as figuras \typeref{figure:MainHighlighterSyntaxTree,figure:MainHighlighterAbstractSyntaxTree} quanto o \typeref{code:MainHighlighterPyResult} representam os mesmos dados,
mas mostrados de formas diferentes (figura \textit{versus} textual) e
com níveis de detalhe diferentes.
...
os nós~=folhas são mostrados com simplificações para que a figura possa ser vista em uma única tela.
\begin{code}
\caption{Arquivo HTML gerado pelo programa de exemplo ``\mintinline{text}|main_highlighter.py|''}
...
\begin{figure}[!htb]
\caption{Árvore Sintática ``\mintinline{text}|main_highlighter_syntax_tree.png|''}
...
\begin{figure}[!htb]
\caption{Árvore Sintática Abstrata ``\mintinline{text}|main_highlighter_abstract_syntax_tree.png|''}
...
\begin{code}
\caption{Resultado da execução do arquivo ``\mintinline{text}|source/main_highlighter.py|''}
...
\item 1 testes de Formatação de Código (\typeref{code:codeFormatterPy}) na classe testes ``\mintinline{text}|TestCodeFormatterBackEnd|''.
\end{enumerate}%
...
\end{code}
\begin{code}
\caption{Arquivo ``\mintinline{text}|source/unit_tests.py|''}
...
A seguir pode~=se encontrar algumas regras semânticas implementadas pelo Analisador Semântico (\typeref{code:semanticAnalyzerPy}).
...
\begin{code}
\caption{Arquivo ``\mintinline{text}|source/semantic_analyzer.py|''}
...
\begin{code}
\caption{Arquivo ``\mintinline{text}|source/code_highlighter.py|''}
...
\begin{code}
\caption{Arquivo ``\mintinline{text}|source/code_formatter.py|''}
...
__file__ )})) e
seu recebimento pelo Analisador Lark (Veja a \fullref{section:GrammarsGrammar}).
Nesta gramática,
...
\begin{code}
\caption{Arquivo ``\mintinline{text}|source/grammars_grammar.pushdown|''}
