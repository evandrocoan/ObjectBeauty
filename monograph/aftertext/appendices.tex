...
encontram~=se as imagens geradas pelo programa ``main\_formatter.py'' (\typeref{code:MainFormatterPy}).
...
encontram~=se as imagens geradas pelo programa ``main\_highlighter.py'' (\typeref{code:MainHighlighterPy}).
...
\begin{enumerate}[i)]
\item 16 testes do Analisador Semântico (\typeref{code:semanticAnalyzerPy}) na classe testes ``TestSemanticRules'';
\item 5 testes de Adição de Cores (\typeref{code:codeHighlighterPy}) na classe testes ``TestCodeHighlighterBackEnd'';
\item 1 testes de Formatação de Código (\typeref{code:codeFormatterPy}) na classe testes ``TestCodeFormatterBackEnd''.
\end{enumerate}%
...
A classe ``TestSemanticRules'' (\typeref{code:unitTestsPy}),
...
criado para verificar tal regra semântica na classe de testes ``TestSemanticRules'' (\typeref{code:unitTestsPy}).
