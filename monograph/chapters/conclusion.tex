...
Ferramentas de formatação de código~=fonte usualmente não permitem que usuários finais tenham o controle total das alterações no código~=fonte.
...
a não ser em casos de \textit{bugs} na ferramenta \cite{findingRegressionsInProjects,debuggingIntoExamples}.
As ferramentas de formatação de código~=fonte em geral,
tentam reconstruir a Árvore Sintática do programa de entrada a ser formatado (\fullref{section:trabalhosRelacionados}).
Neste trabalho,
...
como a restrição do uso da chave de abertura de blocos da linguagem ``\mintinline{text}|Golang|''.
...
não seria muito interessante que existisse uma interface gráfica como a da ferramenta de \citeonline{universalIndentGUI},
...
Enquanto que a criação de uma ferramenta de formatação com uma interface gráfica como a de \citeonline{universalIndentGUI} apresenta problemas (\fullref{section:trabalhosRelacionados}).
...
No exemplo apresentado (\fullref{section:softwareImplementationVisaoGeral}),
...
avaliar se esta nova estrutura de dados é capaz de substituir utilização do caractere ``\mintinline{text}|§|'' (\textit{símbolo de seção}) em conjunto com a classe padrão de \textit{string} da linguagem Python (aumentado a eficiência dos algoritmos de análise do programa de entrada a ser formatado (\fullref{section:desempenhoDoFormator}).
\item Implementar uma interface de linha de comando (CLI, \fullref{section:softwareImplementationVisaoGeral}) que:
\begin{inparaenum}[1)]
...
enquanto ao trabalhar com um formato equivalente como YAML pode ser de mais fácil entendimento (\fullref{section:sourceCodeHighlighters,chapter:firstChapterIntroduction}).
\end{enumerate}%
