...
por que o objetivo deste trabalho é que resolva os problemas apresentados pelas ferramentas de formatação de código~=fonte hoje existentes.
...
avaliar se esta nova estrutura de dados é capaz de substituir utilização do caractere ``§'' em conjunto com a classe padrão de \textit{string} da linguagem Python (aumentado a eficiência dos algoritmos de análise do programa de entrada a ser formatado (\fullref{performanceDoFormator})).
...
XML.
Tal característica seria útil por que algumas linguagem como XML podem ser difíceis de se entender,
