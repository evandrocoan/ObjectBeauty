...
representa a árvore de sintaxe completa da palavra ``token sem nome'',
que a linguagem da gramática no ``\typeref{code:ExemploDeGramaticaLark}'' aceita.
Como pode ser facilmente percebido,
ela é uma árvore de sintaxe estranha.
Cadê o \textit{token}?
O \textit{token} da árvore foi descartado pelo analisador lark,
por que ele não está dentro de uma produção de \textit{token}.
Este é um recurso do analisador lark,
que permite descartamos da árvore de sintaxe \textit{tokens} que julgam~=se desnecessários aparecem na árvore de sintaxe.
\begin{figure}[H]
...
no qual correspondem exatamente os símbolos do alfabeto da linguagem.
...
\end{code}
\begin{figure}[H]
\caption{Árvore do Programa ``palavra12''}
...
\end{figure}
\begin{figure}[H]
\caption{Árvore do Programa ``palavra1''}
...
\end{figure}
\begin{figure}[H]
\caption{Árvore do Programa ``palavra2''}
...
\end{figure}
\begin{figure}[H]
\caption{Árvore do Programa ``palavra3''}
