...
De acordo com o funcionamento de muitos Formatadores de Código (\fullref{chapter:source_code_beautifiers}) e
da ferramenta que foi desenvolvida (\fullref{chapter:software_implementation}),
...
\textit{vice-versa} (\fullref{figure:LinguagensDeterministicas}).
...
Veja o \fullref{chapter:manualDoFormatadorDeCodigo} para uma descrição mais detalhada das informações que apareceram nos \textit{tokens} das Árvores Sintáticas nos exemplos anteriores (\typeref{figure:ArvoresDosTokens1,figure:ArvoresDosTokens2,figure:ArvoresDosTokens3,figure:palavra12,figure:palavra1,figure:palavra2,figure:palavra3}).
...
Earley (\fullref{section:classesDeComplexidade}).
Por fim,
...
complexidade linear ao tamanho da palavra de entrada (\fullref{section:gramaticasVersusLinguagens}).
...
não a linguagem.
\advisor{}{Veja \citeonline{complexityClasses,computationalComplexityAuroraBarak},
...
Polinomial,
Exponential e
Infinita são explicados na \fullref{section:classesDeComplexidade}.}
...
instruções necessários para executar determinado procedimentos (\fullref{section:classesDeComplexidade}).
...
a Máquina de Turing não possui parada garantida\advisor{}{\footnote{
...
sua implementação pode ser representada por algoritmos\advisor{}{\footnote{
...
como mostrado na seção anterior (\fullref{section:analiseSemantica}),
...
é que ela tem desempenho \textit{garantida} pela sua Classe de Complexidade (\fullref{section:classesDeComplexidade}).
