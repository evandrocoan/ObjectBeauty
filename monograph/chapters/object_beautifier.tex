...
podendo reconhecer todas as gramáticas livre de contexto (\fullref{classesDeComplexidade});
...
confira a \fullref{metalinguagemGrammar}.
}'',
capaz que pegar somente o elemento ``\mintinline{text}|if(args.length > 0)|'' de um programa de entrada de qualquer linguagem,
como o exemplo em Java apresentado no \typeref{code:exemploDeUmProgramaNaLinguagemJava}.
Inicia~=se a com a apresentação de uma simplificação da metagramática utilizada neste trabalho.
A metagramática completa pode encontrada no \typeref{chapter:codigoDaMetagramatica}.
Ela segue as mesmas regras do Analisador Lark explicadas na \fullref{introducaoGramaticas}.
Uma melhor descrição do uso dos operadores ``match'' e
``scope'' definidos por essa metagramática,
podem ser encontrados mais tarde na \fullref{metalinguagemGrammar}.
\begin{code}
...
Agora,
o que pode ser feito com a gramática do \typeref{code:exemploDeGramaticaPelasRegrasDaMetagramatica}?
...
que irá devolver como resultado a Árvore Sintática (\fullref{compiladoresEtradutores}).
Então envia~=se a Árvore Sintática para o Analisador Semântico (\typeref{analisadoresSintaticos,analiseSemantica}) verificar se o programa (gramática) de entrada está semanticamente correta,
para então devolver como resultado a Árvore Sintática Abstrata\footnote{%
Do inglês (AST),
Abstract Syntax Tree.
}
(\typeref{desempenhoDoFormator,section:AnalisadorSemantico}).
...
(veja o \typeref{code:Abstract_syntax_tree_for_Euclidean_algorithm,figure:Abstract_syntax_tree_for_Euclidean_algorithm})}.
Veja no \typeref{chapter:execucaoDeMainFormatter} um exemplo de uma Árvore Sintática e
...
Este exemplo de uso do Analisador Lark é igual ao já apresentado na \fullref{introducaoGramaticas},
...
a gramática de entrada no \typeref{code:resultadoDoReconhecimentoDoProgramaJava} não consome todo o programa de entrada (\typeref{code:exemploDeUmProgramaNaLinguagemJava}) no final do processo de reconhecimento (\typeref{code:resultadoDoReconhecimentoDoProgramaJava}).
...
atribuíndo escopos (\typeref{section:sourceCodeHighlightersGrammars}) para cada um deles.
Uma vez que o trecho de código~=fonte ``removido'' possui um escopo atribuído,
um arquivo de configurações JSON como \typeref{code:exemploDeConfiguracaoUtilizadaMinimaDoFormatadorDeCodigo} é consultado.
Caso exista uma ``configuração'' relacionada ao escopo recém~=reconhecido,
o trecho de código~=fonte é enviado para um formatador especializado de código~=fonte como \typeref{code:exemploMinimoDeFormatadorDeCodigo}.
\begin{code}
...
Em trabalhos futuros a este (\fullref{chapter:trabalhosFuturos}) se necessita criar maiores abstrações que permitam trabalhar com gramáticas de linguagens utilizado uma pilha de múltiplos contextos (\fullref{metalinguagemGrammar}),
...
exemplos de uso destes algoritmos nos \typeref{chapter:execucaoDeMainFormatter,chapter:codigoDoFormatador}.
...
Utilizando um Analisador (\typeref{MyWorflowForLarkTraduzido}),
é escrita a gramática da linguagem para qual se quer construir um compilador (\fullref{introducaoGramaticas}).
Após isso,
...
Diferente da \typeref{MyWorflowForLarkTraduzido},
na \typeref{MyWorflowForLarkTraduzido2} é adicionado mais uma etapa ao uso do Analisador.
...
Em vez de permitir que o usuário final da aplicação opere diretamente com o analisador da \typeref{MyWorflowForLarkTraduzido},
...
utilizado para analisar uma gramática LALR(1) (\typeref{chapter:codigoDaMetagramatica}).
Na \typeref{ParsersPublicAudienceTraduzido},
...
entretanto,
a \typeref{MetacompilerMetagrammarMetaprogram} é uma versão mais resumida de como o processo de análise léxica e
sintática acontece.
...
Como já explicado no início desta seção (\typeref{GrammarsGrammar}),
a ligação direta entre a gramática e
a palavra da linguagem não precisa acontecer diretamente com analisador \typeref{MetacompilerMetagrammarMetaprogram}.
...
Como já inicialmente aprendizado na \typeref{section:sourceCodeHighlightersGrammars},
na \typeref{TexMateScopes} é mostrado na primeira linha o trecho de código~=fonte ``\mintinline{text}|function f1 () {|'' e
...
o operador ``include'' está incluindo contextos (\typeref{metalinguagemGrammar}) da gramática atual que serão definidos mais tarde neste mesmo arquivo.
...
contrário da formatação total de código~=fonte como acontece nos demais trabalhos (\fullref{desempenhoDoFormator}).
...
o Analisador Lark entrega a Árvore Sintática da gramática da linguagem sendo descrita pelo metaprograma (\typeref{MetacompilerMetagrammarMetaprogram}).
...
Um Diagrama de Classes é apresentado na \typeref{CodeFormatterClassDiagram}.
Nele,
...
A visita dos nós da Árvore Sintática pela função ``transform(tree)'' acontece simplesmente chamando os métodos que a classe ``TreeTransformer'' (Definido no \typeref{semanticAnalizerConstructor}) define e
...
Continuamente chamar diversos algoritmos independentes acarreta uma perda de desempenho em comparação com os formatadores de código~=fonte apresentados no \fullref{source_code_beautifiers}.
...
como acontece com o formatador Uncrustify (\fullref{trabalhosRelacionados}).
...
Enquanto ``Interpreter'' é responsável por somente ``passear'' pela Árvore Sintática Abstrata,
a classe ``AbstractFormatter'' (\typeref{construtorDeParsedProgram}) é responsável por realmente fazer a formatação de código~=fonte de acordo com a instruções vindas da Árvore Sintática Abstrata.
No final do processo,
