...
Então envia~=se a Árvore Sintática para o Analisador Semântico (\typeref{section:analisadoresSintaticos,section:analiseSemantica}) verificar se o programa (gramática) de entrada está semanticamente correta,
para então devolver como resultado a Árvore Sintática Abstrata\footnote{%
Do inglês (AST),
Abstract Syntax Tree.
}
(\typeref{section:desempenhoDoFormator,section:AnalisadorSemantico}).
...
Em trabalhos futuros a este (\fullref{section:trabalhosFuturos}) se necessita criar maiores abstrações que permitam trabalhar com gramáticas de linguagens utilizado uma pilha de múltiplos contextos (\fullref{section:metalinguagemGrammar}),
...
Em vez de permitir que o usuário final da aplicação opere diretamente com o analisador da \typeref{figure:MyWorflowForLarkTraduzido},
foi criado uma nova metagramática (uma gramática de gramáticas) como mostrado nas \typeref{figure:MyWorflowForLarkTraduzido2}.
...
Como já explicado no início desta seção (\typeref{section:GrammarsGrammar}),
a ligação direta entre a gramática e
a palavra da linguagem não precisa acontecer diretamente com analisador \typeref{figure:MetacompilerMetagrammarMetaprogram}.
...
o operador ``include'' está incluindo contextos (\typeref{section:metalinguagemGrammar}) da gramática atual que serão definidos mais tarde neste mesmo arquivo.
...
\nameref{section:metalinguagemGrammar}.
...
A visita dos nós da Árvore Sintática pela função ``transform(tree)'' acontece simplesmente chamando os métodos que a classe ``TreeTransformer'' (Definido no \typeref{code:semanticAnalizerConstructor}) define e
...
o Formatador de Código faz herança do tipo ``Interpreter'' em vez de ``Transformer'' (\typeref{figure:CodeFormatterClassDiagram}).
A diferença é simples,
``Interpreter'' visita a árvore partindo das folhas até chegar na raíz visitando todos os nós filhos (\fullref{section:compiladoresEtradutores}),
...
a classe ``AbstractFormatter'' (\typeref{code:construtorDeParsedProgram}) é responsável por realmente fazer a formatação de código~=fonte de acordo com a instruções vindas da Árvore Sintática Abstrata.
No final do processo,
