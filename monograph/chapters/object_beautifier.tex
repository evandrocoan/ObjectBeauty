...
por meio de uma metagramática para então formatar o código da linguagem descrita pela gramática.
...
primeiro especifica~=se uma metalinguagem que será utilizada pelos usuários da nova ferramenta de formatação.
...
metalinguagem.
Metaprogramas fazer como parte da entrada do metacompilador junto com a metagramática para gerar um novo compilador.
Neste trabalho,
os metaprogramas serão as gramáticas que serão utilizadas pelos formatadores de código~=fonte.

Os metaprogramas (ou gramáticas) são entradas diretas do metacompilador (a ferramenta Lark na \typeref{MyWorflowForLarkTraduzido2}).
Na \typeref{ParsersPublicAudienceTraduzido},
...
sintáticos nas gramáticas de entrada serão mostrados pela ferramenta Lark.
Isso acontece por que as gramáticas (ou metaprogramas) são entradas diretamente na ferramenta Lark.
...
foi utilizado a ferramenta ``Lark'' \cite{larkContextualLexer} como um metacompilador ou
...
a classe ``ParsedProgram'' \typeref{construtorDeParsedProgram} é responsável por realmente fazer a formatação de código de acordo com a instruções vindas da Árvore de Sintaxe Abstrata.
No final do processo,
