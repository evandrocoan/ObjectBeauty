...
substituíndo eles por algum outro caractere de marcação (como ``\mintinline{text}|§|'') para que não altere~=se o tamanho original do programa de entrada.
...
e gerar código~=binário executável (\typeref{section:compiladoresEtradutores}).
}
...
\end{enumerate}%
\begin{code}
\caption{Simbolo Inicial da Metagramática ``\mintinline{text}|ObjectBeauty|''}
...
Por exemplo,
a palavra ``\mintinline{text}|function|'' possui simultaneamente os escopos:
\begin{inparaenum}[1)]
\item ``\mintinline{text}|source.js|'';
\item ``\mintinline{text}|meta.function.js|'' e;
\item ``\mintinline{text}|storage.type.function.js|''.
\end{inparaenum}%
Enquanto o caractere ``\mintinline{text}|{|'' possui simultaneamente os escopos:
\begin{inparaenum}[1)]
\item ``\mintinline{text}|source.js|'';
...
são mostrados mais alguns exemplos de uso do operador ``\mintinline{text}|match|'' classificando diversos tipos de numéricos (da linguagem sendo descrita pela gramática).
...
A visita dos nós da Árvore Sintática pela função ``\mintinline{text}|transform(tree)|'' acontece simplesmente chamando os métodos que a classe ``\mintinline{text}|TreeTransformer|'' (Definido no \typeref{code:semanticAnalizerConstructor}) define e
...
Mesmo que não existam os métodos ``\mintinline{text}|preamble_statements|'' e
``\mintinline{text}|language_construct_rules|'' definidos na classe ``\mintinline{text}|TreeTransformer|'',
...
mantém a variável ``\mintinline{text}|parent|'',
...
``\mintinline{text}|Interpreter|'' também recebe diretamente no construtor qual será a árvore que ele irá navegar sobre.
Nos demais aspectos,
...
\begin{code}
\caption{Construtor de ``\mintinline{text}|AbstractFormatter|''}
...
os caracteres que foram consumidos pela gramática são substituídos pelo caractere ``\mintinline{text}|§|'' para impedir que eles sejam reconsumidos.
A utilização do caractere ``\mintinline{text}|§|'' foi realizada para simplificar a implementação do algoritmo de consumo.
Com o caractere ``\mintinline{text}|§|'' colocado no lugar dos caracteres que já foram consumidos,
