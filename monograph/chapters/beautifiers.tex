...
\lang{%
    A robust Code Beautifier can get a lot more complicated just with the basic definitions of formatting (\typeref{chapter:source_code_beautifiers}) applied over each language own characteristics as for example:
...
\lang{%
    ...
    \textit{C++}, \textit{C\#} e
    \textit{ObjectiveC} (veja mais linguagens na \fullref{section:trabalhosRelacionados}).
...
Nesta seção,
são relatados os trabalhos relacionados obtidos através de pesquisa na literatura utilizando a metodologia especificada no \typeref{chapter:firstChapterIntroduction}.
...
a versão ``0.69.0'' possui 671 configurações para personalizar a formatação do código~=fonte como mostrado no \typeref{code:uncrustifySettingSample}.
...
Como já mencionado na \typeref{section:trabalhosRelacionados},
...
Por exemplo,
diferente da linguagem \textit{JavaScript} utilizada no \typeref{frame:exemploDeOfuscador},
