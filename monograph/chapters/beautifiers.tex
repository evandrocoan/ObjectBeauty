...
Também é possível utilizar um único arquivo ``.sublime-syntax'' com diversas configurações de cores ou
estilos diferentes (``.sublime-color-scheme'').

Como o código~=fonte do editor \citeonline{sublimeTextSyntax} é proprietário,
\citeonline{rustSublimeTextSyntaxSyntec} fez a implementação de uma versão de código~=aberto que reconhece os arquivos de gramática (``.sublime-syntax'') e
configurações de estilos (``.sublime-color-scheme'') do editor \citeonline{sublimeTextSyntax}.
...
arquivos de estilos criados pelos seus usuários.
Assim,
...
estilos utilizados por \citeonline{vsCodeSyntaxHighlighthing} e
...
\citeonline{sublimeTextSyntax} continua compatível com os arquivos de gramáticas e
estilos originais de \citeonline{textMateEditor},
...
portanto,
tradutores (\fullref{compiladoresEtradutores}) como \citeonline{packageDevSublime} podem ser escritos sem dificuldade para converter uma gramática de \citeonline{textMateEditor} para \citeonline{sublimeTextSyntax}.
...
estilos utilizados por \citeonline{sublimeTextSyntax} por que dos três trabalhos:
\begin{inparaenum}[1)]
...
\end{inparaenum}%
No \typeref{exemploDeUmArquivoSublimeSyntax},
...
\textit{while},
\end{inparaenum}%
com o ``escopo \mintinline{text}|keyword.control.c|'' de uma certa linguagem chamada ``C'',
...
Uma vez que o \typeref{exemploDeUmArquivoSublimeSyntax} classificou as palavras~=chave com o ``escopo \mintinline{text}|keyword.control.c|'',
...
No \typeref{exemploDeUmArquivoSublimeColorScheme},
encontra~=se um arquivo JSON de configuração de estilos.
Nele,
...
As demais especializações de escopos do arquivo (\typeref{exemploDeUmArquivoSublimeColorScheme}) não foram utilizadas porque a gramática (\typeref{exemploDeUmArquivoSublimeSyntax}),
...
\begin{code}
\caption{Exemplo de um arquivo ``.sublime-color-scheme''}
\label{exemploDeUmArquivoSublimeColorScheme}
\begin{minted}{json}
{
...
Por essas e
outras,
foram criados guias de estilos \cite{sublimeTextScopeNaming} onde criam~=se convenções de nomes de escopos para serem utilizados tanto por quem cria gramáticas,
quanto por que cria arquivos de estilos para editores de texto.

Mas porque exitem arquivos de estilos?
...
No próximo capítulo será apresentado a união do estado da arte dos formatadores de código~=fonte,
