...
\label{chapter:firstChapterIntroduction}
...
\lang{%
    ...
    do tipo ``\mintinline{text}|if|'', ``\mintinline{text}|for|'',
    etc.
    } Já um outro ``Programador B'',
    possui o costume oposto,
    ele jamais deixa uma linha em branco antes dessas estruturas de controle.
    Então,
    quais problemas podem acontecer quando ambos os programadores ``A'' e
    ``B'' trabalham em um mesmo projeto?
    ...
    como ``\mintinline{text}|if|'' ou
    ``\mintinline{text}|for|''} de acordo com a presença ou
...
\lang{%
    ...
    and when he was writing your code involving an ``\mintinline{text}|if|'',
...
\advisor{}{%
    \lang{%
        ...
        \begin{enumerate}
            ...
            Por exemplo,
            suponha que a sua tecla ``\mintinline{text}|TAB|'' não funcione.
            Assim,
            ao invés de somente pressionar ``\mintinline{text}|TAB|'' uma vez para indentar seu código~=fonte,
            você ganha habito de pressionar a tecla de espaços 4 vezes.
            Portanto,
            nasce um motivo para alguém utilizar espaços ao invés ``\mintinline{text}|TAB|\s'' para realizar a indentação de seu código~=fonte.
        \end{enumerate}
    }
}
...
\lang{%
    ...
    é essencial utilizar um sistema de controle de versões (versionamento) \cite{whyVersionControlIsCritical}.
    \advisor{%
    ...
    por exemplo,
    através de uma ``\textit{commit}'' como conhecidas em sistemas \textit{git} \cite{usingSourceControl}.
    }%
...
\lang{%
    ...
    not the addition of new blank lines \cite{findingRegressionsInProjects,debuggingIntoExamples}.
}{%
    \advisor{
        Ao trabalhar com um sistema de controle de versões como \textit{git},
        ...
        fazendo mudanças insignificantes como inserindo novas linhas antes de cada ``\mintinline{text}|if|'',
        ...
        Um bom uso de um sistema de controle de versões é somente conter mudanças que sejam significativas,
        como criações de novas funções,
        correção de \textit{bugs} \cite{findingRegressionsInProjects,debuggingIntoExamples},
        etc.
        Ferramentas de controle de versões auxiliam na manutenção,
        rastreabilidade de responsabilidades e
        alterações,
        mas não apoiam na manutenção de um estilo de formatação único para uma equipe de programadores.
    }{
    ...
    \begin{enumerate}
        ...
        pois o foco de um sistema de controle de versões são conter mudanças se sejam significantes,
        como criações de novas funções,
        correção de \textit{bugs} \cite{findingRegressionsInProjects,debuggingIntoExamples},
        etc.
    \end{enumerate}
...
\lang{%
    ...
    desenvolver uma ferramenta de reorganização do código~=fonte nos aspectos estruturais ou
    sintáticos (veja o último exemplo da \fullref{section:introducaoGramaticas}).
...
Ter que aprender e
configurar muitas ferramentas de formatação de código~=fonte é um tarefa cansativa (\fullref{section:formatadores_de_codigo}).
Mais ainda,
...
\advisor{%
    Esta seção apresenta sucintamente os objetivos deste trabalho,
...
extensível a diversas linguagem de programação por meio de uma nova definição gramáticas,
especificada por uma metagramática.
...
Destes objetivos específicos,
todos eles foram alcançados como pode ser observado dos \fullref{chapter:fundamentacaoTeorica,chapter:source_code_beautifiers,chapter:software_implementation,chapter:conclusion}.
...
No \fullref{chapter:fundamentacaoTeorica},
...
No \fullref{chapter:source_code_beautifiers},
...
No \fullref{chapter:manualDoFormatadorDeCodigo},
