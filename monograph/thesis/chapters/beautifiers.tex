...
\lang{
    ...
    veja mais sobre configurações na \fullref{sectionUncrustify}.
}
...
\lang{%
    ...
    veja mais linguagens na \fullref{sectionUncrustify}.
    ...
    Ao realiza~=se a migração de uma ferramenta de formatação como \textit{Uncrustify} para outra,
    precisa~=se configurar novamente todas as novas opções já definidas para \textit{Uncrustify}.
...
\subsection{Uncrustify}
\label{sectionUncrustify}

\textit{Uncrustify}, \citeonline{uncrustifySourceCode} é capaz de formatar as linguagens \begin{inparaenum} \item C,
\item C++,
\item C\#,
\item ObjectiveC,
\item D,
...
Tais características enfraquecem o poder do ofuscamento uma vez que ele possui um maior efeito quando pode~=se introduzir mair caos no documento,
...
renomeando variáveis para nomes obtusos como ``A88''.
