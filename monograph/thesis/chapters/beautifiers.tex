...
Uncrustify},label={uncrustifySettingSample}]
...
na versão ``0.69.0'' existem 671 configurações para personalizar a formatação do código~=fonte como mostrado no \typeref{uncrustifySettingSample}.
...
entusiastas por novas tecnologias podem utilizar simultaneamente duas ou
mais ferramentas dependendo de suas especialidades e
da tarefa que o desenvolvedor está realizando no momento.

Trabalhos como \cite{editorConfig} não são diretamente um formatador de código~=fonte,
...
Do mesmo jeito,
este trabalho foca em propor uma ferramenta de formatação extensível várias linguagens de programação (ver \fullref{software_implementation}).
...
Ela permite que se escolha qual será o formatador de código que utilizado,
...
Mas a interface gráfica de \citeonline{universalIndentGUI} não possui nem próximo de tantas opções de configuração.

Todas as opções que não aparecem na interface gráfica,
não podem ser utilizadas ao realizar a formatação do código~=fonte por que os arquivos de configuração gerados e
lidos pela ferramenta não conterão estas opções e
qualquer outra opção além inserido pelo usuário manualmente serão apagadas.
Assim não podendo ser utilizadas mais opções além das quais a limitada interface gráfica mostra.

Outra desvantagem da visualização das mudanças como mostrado na \typeref{UniversalIndentGUIScreenshot4},
...
Neste caso,
não serão apresentados nenhuma mudança durante a visualização.
