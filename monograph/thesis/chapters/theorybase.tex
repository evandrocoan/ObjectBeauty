...
\subsection{Hierarquia de Chomsky}
    ...
    *Para Gramáticas Regulares Determinísticas,
    ...
    Automatos Finitos Não~=Determinísticos ou
    Analisadores com Backtracking,
    tempo Exponencial ao tamanho da Gramática de Entrada.

    **Para Gramáticas Livres de Contexto Determinísticas,
    também conhecidas como LR(K),
    complexidade Linear ao tamanho da palavra de entrada (veja \fullref{gramaticasVersusLinguagens}).
    Para Gramáticas Livre de Contexto Não~=Determinísticas,
    complexidade Exponencial construir a Árvore de Derivações utilizado Backtracking e
...
\label{classesDeComplexidade}
    ...
    Gramáticas Não~=Determinísticas possuem complexidade Exponential\footnote{
    Entretanto,
    Gramáticas Livre de Contexto Não~=Determinísticas também podem ser Analisadas em tempo Polinomial quando utilizados algoritmos como CYK \cite{hopcroftBook}.
...
\subsection{Complexidade Teórica $versus$ Real}
    ...
    implementado através de algum algoritmo com tempo Polinomial.
    ...
    ou como também chamada de Árvores Sintática por tratar~=se de um Gramáticas Livre de Contexto que trata de Aspectos Estruturais de Linguagens.

    Algoritmos de construção das Árvores de Derivação para Gramáticas Livre de Contexto Não~=Determinística,
