...
\label{analisadoresSintaticos}
    ...
    \citeonline{lalrDeRemer1982} criaram os Analisadores LALR(K)\footnote{\textit{Look~=Ahead} LA(K) LR(0),
    ...
    Note que,
    a pesar disso não impede~=se que a Gramática da Linguagem C seja analisada com eficiência,
...
\subsection{Complexidade Teórica \textit{versus} Real}
    ...
    O tempo de execução será Polinomial ao tamanho da entrada caso o Analisador Não~=Determinístico seja de uma linguagem Livre de Contexto Ambígua (Não~=Determinística) e
    ...
    Não~=Determinísticas são equivalentes pois sempre é possível simular o funcionamento de uma Máquina de Turing Não~=Determinística,
    utilizando uma Máquina de Turing Determinística \cite{hopcroftBook}.


\subsection{Busca em Largura e em Profundidade}
    ...
    Já Analisadores Não~=Determinísticos executam simultaneamente todos os ramos de computação não~=determinísticos,
    conseguindo assim, desempenho Linear ou
    Polinomial ao tamanho da entrada \cite{hopcroftBook}.
    \begin{figure}[H]
    \centering
    ...
    A Figura \ref{fig:pictures/ParserDeterministic.png} mostra um exemplo de uso do algoritmo de Busca em Profundidade.
    ...
    ou seguir executando todos os ramos que fazem parte de um mesmo nível de computação.
}
