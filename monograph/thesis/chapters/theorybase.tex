...
\label{analisadoresSintaticos}
    ...
    Máquinas de Turing.
    Os Analisadores Sintáticos realizam o reconhecimento da linguagem recebendo diretamente como entrada a Gramática da Linguagem.
...
\subsection{Gramáticas $versus$ Linguagens}
    ...
    É importante notar que o processor de análise por um Analisador,
    seja ele LR(K) ou
    LL(K),
    acontece em duas etapas.
    Primeiro utilizam~=se algoritmos de construção da Tabela de Análise.
    Uma vez que esta tabele está construída sem conflitos,
    entra em cena o algoritmos de análise,
    que utilizando a tabela de análise,
    realiza o reconhecimento do programa de entrada.


\subsection{Analisadores LALR(K) e SLR(K)}
