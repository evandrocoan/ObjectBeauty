...
De acordo com o funcionamento de muitos Formatadores de Código (veja \fullref{source_code_beautifiers}) e
da ferramenta que será desenvolvida (veja \fullref{software_implementation}),
...
\lang{%
    ...
    tem~=se um processo de tradução (\autoref{ProcessoTraducao}).
...
\label{hierarquiaDeChomsky}
    ...
    Irrestrita (\autoref{HierarquiaDeChomsky}).
    \begin{figure}[h]
    \centering
    ...
    ***Para verificar se uma dada sentença pertence ou não a linguagem.
    ...
    Sensíveis ao Contexto como apresentado na \autoref{HierarquiaDeChomsky}.
    Por isso,
...
\label{gramaticasVersusLinguagens}
    ...
    reconhecem um subconjunto das Linguagens Livre de Contexto (\autoref{LinguagensDeterministicas}).
    ...
    A \autoref{LinguagensDeterministicas} não é inteiramente um Diagrama de Venn \cite{generalizedVennDiagrams},
    inicialmente,
    ...
    LR(K).
    Reveja os parágrafos após a \autoref{HierarquiaDeChomsky}
    },
    ...
    \citeonline{llVersusLrContainment,llContainmentInLalr,beatty1982,ahoCompilerDragonBook,grammarsAndTheMinimalParsers}}
    ...
    reveja a \fullref{hierarquiaDeChomsky}
    }
...
\subsection{Analisadores LR(K)}
    ...
    Como pode ser observado na \autoref{LinguagensDeterministicas},
    ...
    Como mostrado na \autoref{LinguagensDeterministicas},
...
\label{alteracoesNosAnalisadoresSintaticos}
    ...
    reveja a \autoref{HierarquiaDeChomsky} e
...
\subsection{Complexidade Teórica $versus$ Real}
    ...
    Na Figura \autoref{ParserNonDeterministic},
    ...
    Como mostrado na Figura \autoref{ParserNonDeterministic},
    ...
    Veja as Figuras \autoref{ParserNonDeterministic,ParserDeterministic} e
    as compare.
    ...
    implementado através de algum algoritmo com tempo polinomial como CYK (\fullref{hierarquiaDeChomsky}).

    Como já explicado nas observações da \autoref{HierarquiaDeChomsky},
...
\label{mecanismosReconhecedores}
    ...
    Na \autoref{ParserDeterministic},
    encontra~=se a mesma Árvore de Computação apresentada na \autoref{ParserNonDeterministic},
    ...
    \begin{figure}[h]
    \centering
    \includegraphics[width=1.0\textwidth]{ParserDeterministic.png}
    ...
    com a exceção de algums trabalhos como \citeonline{efficientNonDeterministicParsers} que tenta utilizar computação paralela para representar o Não~=Determinismo.
    }
...
\label{buscaEmLarguraEProfundidade}
    ...
    A \autoref{ParserDeterministic} mostrou um exemplo de uso do algoritmo de Busca em Profundidade.
    Já na \autoref{ParserDeterministicBreadth},
