...
\lang{%
    ...
    Muitos projetos podem \advisor{}{sempre} ser escritos pela mesma pessoa durante toda a vida desse projeto mas,
    ...
    Então,
    quando diferentes programadores co~=operam em grandes projetos,
    \advisor{como unir pessoas com experiências e
    preferências diferentes?}{%
    como essas pessoas todas poderiam trabalhar juntas,
    mesmo elas não tendo as mesmas experiências?} \advisor{Uma}{Assim,
    uma} barreira adicional \advisor{ao}{para cada um desses programadores no} desenvolvimento do projeto
    ...
    branco antes de cada estrutura de controle\advisor{.}{ do tipo ``if'',
    ``for'',
    etc.
    ...
    \advisor{O problema mais provável é que o código-fonte esteja escrito em dois estilos:
    do Programador A e
    B.
    ...
    \advisor{há uma dificuldade na leitura do código como algo homogêneo.
    Os próprios Programadores A e
    B, autores do código, teriam dificuldades,
    pois ele segue um estilo diferente ao que A e
    B estão habituados.
    }{certas heurísticas tem sua taxa de acerto reduzida ao fazer o reconhecimento dos padrões.
    } Um exemplo de padrão que estaria prejudicado seria rapidamente
    identificar onde pode estar uma estrutura de controle,
    \advisor{}{como ``if'' ou ``for''} de acordo com a presença ou não de uma linha em branco antes dela,
...
\lang{%
    ...
    uma vez que a sua organização difere dos padrões no qual ele sabe como rapidamente se localizar.
    \cite{quantifyingProgramComprehension}
    }
}
...
\lang{%
    ...
    As diferenças entres os diversos estilos de programação entre
    os programadores deve\hyp{}se à diversos fatores:
    \begin{enumerate}
        \item Alguns programadores simplesmente fazem de um determinado
        \advisor{estilo devido as suas experiências iniciais}{jeito por
        que foi assim que eles aprenderam de seus tutores} e
        nunca preocuparam\hyp{}se em questionar se poderia ser feito de outra maneira.
        Um caso onde isso pode acontecer é devido ao cansaço \advisor{da tarefa de programar.%
        }{que algumas linguagens de programação como \latex causam por sua sintaxe incomum e
        extraordinária.} \advisor{Estando cansado,
        um programador pode apresentar problemas de concentração,
        como qualquer outro profissional,
        e consequentemente,
        não conseguir manter organização e
        legibilidade no código.}{Uma vez que você já está cansado de ver tantos errors,
        ...
        de seus códigos devido a algum aspecto emocional ou físico.
        Emocionalmente o programador pode ``não gostar'' de alguma característica por que
        \advisor{}{sofreu algum trauma ou} simplesmente pode estar muito acostumado a ter
        ter visualmente todos os elementos muito próximos um dos outros na tela.
        Assim,
        mesmo tal característica não sendo nada fácil para que outras pessoas entendam o código,
        ela pode ser muito fácil de ser compreendida pelo próprio programador que a escreveu,
        já que o mesmo vem a muitos anos habituado a realizar a
        escrita de seus códigos desta determinada forma,
        portanto é muito hábil com a mesma e
        rapidamente consegue entender o que escreve.

        \advisor{}{\item Uma característica física que pode determinar a
            formatação de código de alguém pode ser um teclado defeituoso.
            ...
            Portanto,
            nasce um motivo para alguém utilizar espaços ao invés ``TAB\s''
            para realizar a indentação de seu código.
        }
    \end{enumerate}
}
...
\lang{%
    ...
    também tem que se preocupar como o código será salvo no sistema de arquivos,
    em seu formato de texto simples.
    Já que para compartilhar código e
    trabalhar em times de forma eficiente,
    é essencial utilizar um sistema de
    ...
    \advisor{Tal ferramenta deve permitir rastrear mudanças \cite{gettingProductive},
    habilitando que se entenda melhor as atividades de cada programador.
    \cite{usingSourceControl}
    }{Tal ferramenta deve permitir facilmente realizar o
    rastreamento das mudanças \cite{gettingProductive} e
    assim,
...
\begin{citacao}
    ...
    \cite[tradução nossa]{Geukens} \footnote{\englishword{I'd say there
...
\lang{%
    ...
    \advisor{
      Estas conclusões seguem porque ao trabalhar com um sistema de versionamento como ``git'',
      recomenda\hyp{}se manter todo o código sobre um único estilo,
      devido aos problemas gerados ao permitir que cada programador escreva como lhe bem entender.
      Por exemplo,
      gerando ruídos de formatação dificultando a revisão do código~=fonte,
      dificultando o entendimento que cada programador \cite{quitDiffCalculating}.

      Caso todos os programadores decidam re~=escrever o histórico,
      fazendo mudanças como inserindo novas linhas antes de cada ``if'':
    }{
        Segue\hyp{}se estas conclusões devido aos problemas gerados ao permitir
        que cada programador escreva como lhe agrada melhor.
        Caso cada um escreva como bem entende,
        irá existir ruídos de formatação em abundância espalhados por tudo o código\hyp{}fonte,
    ...
    \begin{enumerate}
        \item Eles irão desperdiçar um tempo que eles poderiam estar utilizando para escrever código.
...
\lang{%
    ...
    \advisor{}{%
        ...
        só porque algum programador do
        turno da
...
\lang{%
    ...
    \advisor{%
        Cada programador escreve seu código de sua maneira.
        Por exemplo,
        quanto a indentação,
        ...
        Cada programador aprende a escrever seus códigos de uma determinada maneira,
        seja utilizando 2, 4,
...
\lang{%
    ...
    desenvolver uma ferramenta de disposição ou organização do código,
    também conhecidos como ``Source Code Beautifiers'' ou ``Formatadores de Código''.
    Com isso,
    espera\hyp{}se que o desenvolvimento de códigos demande menos trabalho,
    pois o desenvolvedor pode forcar mais seus esforços pensando
    mais sobre o problema que está sendo resolvido,
...
\advisor{%
    Esta seção apresenta os objetivos desse trabalho,
    ...
    \end{enumerate}
        ...
        realizar o desenvolvimento de uma nova ferramenta de formatação de código~=fonte
        que seja extensível por gramáticas fornecidas pelos usuários.
    }
}%

\subsection{Objetivo Geral}

Desenvolver um formatador de código fonte extensível a
qualquer linguagem de programação por meio de gramáticas.
...
\begin{enumerate}
    ...
    \lang{%
        ...
        \item Projetar uma ferramenta que permita várias e
        diferentes linguagens de programação sejam formatadas utilizando o mesmo conjunto de configurações.
    }
    \lang{%
        \item Moreover, explain the differences for other softwares and the benefits
